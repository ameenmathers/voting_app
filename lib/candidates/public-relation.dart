...
class PublicRelationtPage extends StatefulWidget {
  final String name;
  final String photoUrl;
  final bool hasVoted3;
  final String uid;

  const PublicRelationtPage({
    Key key,
    @required this.name,
  ...
  @override
  _PublicRelationtPageState createState() => _PublicRelationtPageState();
}

class _PublicRelationtPageState extends State<PublicRelationtPage> {
  ...
  Future<List<DocumentSnapshot>> getUsersList() async {
    QuerySnapshot querySnapshot = await _usersCollectionReference
        .where('position', isEqualTo: 'Public Relations Officer')
        .getDocuments();
