...
class SoftwareDirectorPage extends StatefulWidget {
  final String name;
  final String photoUrl;
  final bool hasVoted6;
  final String uid;

  const SoftwareDirectorPage({
    Key key,
    @required this.name,
  ...
  @override
  _SoftwareDirectorPageState createState() => _SoftwareDirectorPageState();
}

class _SoftwareDirectorPageState extends State<SoftwareDirectorPage> {
  ...
  Future<List<DocumentSnapshot>> getUsersList() async {
    QuerySnapshot querySnapshot = await _usersCollectionReference
        .where('position', isEqualTo: 'Software Director')
        .getDocuments();
